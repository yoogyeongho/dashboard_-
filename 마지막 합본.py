...
with st.sidebar:
    st.header("📁 데이터 경로")
    final_path  = st.text_input("final_df: final_df.csv", value="final_df.csv")
...
col1.metric("전체 가맹점 수", f"{all_kpi['전체 가맹점 수']:,}")
col2.metric("주의/위험 비중", f"{all_kpi['주의/위험 비중(%)']:.1f}%")
col3.metric("폐업 매장 비중", f"{all_kpi['폐업 매장 비중(%)']:.1f}%")
col4.metric("평균 회복탄력성", f"{all_kpi['평균 회복탄력성']:.1f}")
...
if filtered.empty or (selected_dong is None and selected_category is None):
    for c, title in zip([colf1,colf2,colf3,colf4],["선택 가맹점 수","주의/위험 비중","폐업 매장 비중","평균 회복탄력성"]):
        c.metric(title, "-")
else:
    k = kpi_summary(filtered)
    colf1.metric("선택 가맹점 수", f"{k['전체 가맹점 수']:,}")
    colf2.metric("주의/위험 비중", f"{k['주의/위험 비중(%)']:.1f}%")
    colf3.metric("폐업 매장 비중", f"{k['폐업 매장 비중(%)']:.1f}%")
    colf4.metric("평균 회복탄력성", f"{k['평균 회복탄력성']:.1f}")
