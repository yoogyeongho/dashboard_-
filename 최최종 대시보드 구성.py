...
def load_data():
    ...
    try:
        ...
        st.error("final_df.csv 파일을 찾을 수 없습니다.")
        return pd.DataFrame()
                ...
                with warning_cols[idx]:
                    ...
                    st.markdown(f"""
                        <div class="metric-card">
                            ...
                            <p style="font-size: 1.5rem; margin: 0;">{current_count}</p>
                            <p style="font-size: 0.9rem; margin: 0;">{change_str}</p>
                        </div>
                    """, unsafe_allow_html=True)
    ...
    with charts_container:
        ...
        with col1:
            ...
            fig = go.Figure(data=[go.Pie(
                ...
                hole=.5,
                marker=dict(colors=['#00ff88', '#ffcc00', '#ff8800', '#ff4444']),
        ...
        with col2:
            ...
            html += '<tr><th>등급</th><th>증감률</th><th>가맹점수</th></tr>'
            for _, row in df_table.iterrows():
                color = 'color:#ff4444;' if row['등급'] == '위험' else ''
                html += f'<tr><td>{row["등급"]}</td><td>{row["증감률"]}</td><td style="{color}">{row["가맹점수"]}</td></tr>'
            html += '</table>'
        ...
        with col3:
            ...
            colors = ['#00ff88', '#66ff66', '#ffcc00', '#ff8800', '#ff6600', '#ff4444']
        ...
        with col4:
            ...
            html += '<tr><th>등급</th><th>증감률</th><th>가맹점수</th></tr>'
            for _, row in df_table.iterrows():
                color = 'color:#ff4444;' if row['등급'] in ['5등급', '6등급'] else ''
                html += f'<tr><td>{row["등급"]}</td><td>{row["증감률"]}</td><td style="{color}">{row["가맹점수"]}</td></tr>'
            html += '</table>'
                    ...
                    if unique_franchises_df.empty:
                        ...
                        if not occupation_df.empty and len(occupation_df) > 1:
                            ...
                            fig.add_trace(go.Scatter(
                                ...
                                line=dict(color='#00ff88', width=2, dash='dash', shape='hv'),
                        ...
                        if selected_dong != '전체':
                            ...
                            if not district_df.empty and len(district_df) > 1:
                                ...
                                fig.add_trace(go.Scatter(
                                    ...
                                    line=dict(color='#ff8800', width=2, shape='hv'),
                                    hovertemplate='운영기간: %{x:.1f}개월<br>생존확률: %{y:.2%}<extra></extra>'
                                ))
                        ...
                        if business_danger:
                            ...
                            fig = go.Figure(data=[
                                go.Bar(
                                    ...
                                    textposition='outside',
                                    marker_color=['#ff4444' if i == 0 else '#ff8800' for i in range(len(top_danger))]
                                )
                            ])
                        ...
                        if business_sales:
                            ...
                            fig = go.Figure(data=[
                                go.Bar(
                                    ...
                                    textposition='outside',
                                    marker_color=['#ff4444' if i == 0 else '#ff8800' for i in range(len(top_sales))]
                                )
                            ])
