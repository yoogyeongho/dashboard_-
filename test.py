...
def calculate_warning_grade(df, model, features, le_dict):
    ...
    df['위기 경보등급'] = np.select(conditions, choices, default='안정')
...
def create_ranking_bar_chart(data_df, value_col, name_col, title, color_scale, limit=20):
    ...
    for i in range(len(top_data)):
        if i < 3:
            colors.append(f'rgb({200+i*15}, {30+i*10}, {30+i*10})')
        elif i < 10:
            colors.append(f'rgb({180-i*5}, {50+i*8}, {100+i*10})')
        else:
            colors.append(f'rgb({100-i*2}, {100+i*5}, {150+i*3})')
...
def main():
    ...
    st.markdown(f"<h3 style='text-align:center; color:#1a1a1a;'>📅 {current_date}</h3>", unsafe_allow_html=True)
    ...
    with chart_placeholder.container():
        ...
        with col1:
            ...
            for dong in dong_df['행정동'].unique():
                ...
                danger_count = len(dong_data[dong_data['위기 경보등급'] == '위험'])
                if total_count > 0:
        ...
        with col2:
            ...
            for grade in ['위험', '주의', '관심', '안정']:
                count = len(grade_df[grade_df['위기 경보등급'] == grade])
        ...
        with col3:
            ...
            if st.session_state.selected_dong and st.session_state.selected_grade:
                business_df = current_df[
                    ...
                    (current_df['위기 경보등급'] == st.session_state.selected_grade)
                ]
                # 해당 행정동의 업종별 위험 비율
                business_danger = []
                dong_df = current_df[current_df['행정동'] == st.session_state.selected_dong]
                for business in dong_df['업종'].unique():
                    ...
                    danger_count = len(business_data[business_data['위기 경보등급'] == st.session_state.selected_grade])
                    if total_count > 0:
                        danger_ratio = (danger_count / total_count) * 100
                ...
                for business in current_df['업종'].unique():
                    ...
                    total_count = len(business_data)
                    danger_count = len(business_data[business_data['위기 경보등급'] == '위험'])
                    if total_count > 0:
                        danger_ratio = (danger_count / total_count) * 100
                        business_danger.append({
                            '업종': business,
        ...
        with col4:
            ...
            if st.session_state.selected_dong and st.session_state.selected_grade and st.session_state.selected_business:
                ...
                # 전체 위험 가맹점
                store_df = current_df[current_df['위기 경보등급'] == '위험']
    ...
    if st.session_state.selected_dong or st.session_state.selected_grade or st.session_state.selected_business:
        ...
        st.markdown(f"<p style='color: #666666;'>🔍 선택된 필터: {' > '.join(selected_info)}</p>", unsafe_allow_html=True)
